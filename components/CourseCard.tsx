    ...
    <div className="group flex flex-col bg-white border border-mci-borderGrey rounded-lg overflow-hidden transition-all duration-300 hover:shadow-lg hover:-translate-y-1 h-full">
      <div className="p-6 flex flex-col flex-grow">
        <div className="flex justify-between items-start mb-4">
          <span className="inline-block px-3 py-1 bg-mci-lightGrey text-mci-text text-xs font-bold tracking-wider uppercase rounded-sm">
            {course.category}
          </span>
          <span className="text-mci-teal text-xs font-semibold">{course.code}</span>
        </div>
        
        <h3 className="text-xl font-bold text-mci-text mb-3 group-hover:text-mci-teal transition-colors">
          ...
          <div className="bg-mci-lightGrey/50 -mx-6 -mb-6 px-6 py-4 flex justify-between items-center mt-auto border-t border-gray-100 group-hover:bg-mci-lightGrey transition-colors">
            <div className="flex items-center gap-2">
              ...
              <div className="flex flex-col">
                <span className="text-xs font-bold text-mci-text">Next: {new Date(nextSession.startDate).toLocaleDateString(undefined, { month: 'short', day: 'numeric' })}</span>
                <span className="text-[10px] text-gray-500">{nextSession.location}</span>
              </div>
            </div>
            <Link 
              to={`/courses/${course.id}`} 
              className="w-8 h-8 rounded-full bg-white border border-gray-200 flex items-center justify-center text-mci-text group-hover:bg-mci-amber group-hover:border-mci-amber group-hover:text-white transition-colors"
            >
